...
def bilateral_filter(vid, hyperparams=None):
    if hyperparams is None:
        hyperparams = {"kernel_size": 5, "sigma_xy": 1.0, "sigma_t": 1.0, "sigma_intensity": 0.1}

    kernel_size = hyperparams["kernel_size"]
    sigma_xy = hyperparams["sigma_xy"]
    sigma_t = hyperparams["sigma_t"]
    sigma_intensity = hyperparams["sigma_intensity"]
        ...
        for j in range(vid.shape[1]):
            for k in range(vid.shape[2]):
                if i < kernel_size // 2 or i >= vid.shape[0] - kernel_size // 2 or \
                    ...
                    range_kernel = np.exp(-0.5 * ((patch - vid[i, j, k]) ** 2) / sigma_intensity ** 2)
                    combined_kernel = kernel * range_kernel
                    combined_kernel /= np.sum(combined_kernel)
                    output_vid[i, j, k] = np.sum(combined_kernel * patch)
    return output_vid
